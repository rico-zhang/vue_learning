...
- 除了未被声明过和未被渲染的数据外，还有什么数据更改后不会渲染页面？
  ...
  ```
- 问：要如何响应式的更新数组和对象？
  > 更改数组：<br> 1. 利用数组变异方法：push、pop、shift、unshift、splice、sort、reverse <br> 2. 利用vm.\$set/Vue.set实例方法<br>3. 利用vm.\$set或Vue.set删除数组中的某一项 <br>4. 可以调用vm.$forceUpdate()强制渲染页面) <br>5. 修改一下原来有的属性(此属性在页面中有用到)，那么会重新渲染页面，此时新添加的那个属性也会被渲染到页面上 
...
```js
        //自己实现的版本
        //对象的增删属性 不监听
        //数组的索引访问/设置 length访问不监听 【注意:无论是已存在的索引还是新增的索引，都不监听】
        //但是数组的项如果是对象，对项的属性要监听 如  const data = { arr: [{ id: 1, name: "rico" }] } ,不监听data.arr[0],但是监听data.arr[0].id
        //对数组"push", "pop", "shift", "unshift", "splice", "soft", "reverse"方法重写 监听

        class MyVue {
            constructor(op = {}) {
                this.data = op.data;
                const oldPrototype = Array.prototype;
                this.VueArray = Object.create(oldPrototype);
                ["push", "pop", "shift", "unshift", "splice", "soft", "reverse"].forEach(method => {
                    this.VueArray[method] = function () {
                        var ret = oldPrototype[method].call(this, ...arguments);
                        render();
                        return ret;
                    }
                });
            ...
            Observe(data) {
                if (typeof (data) === "object") {
                    if (Array.isArray(data)) {
                        Object.setPrototypeOf(data, this.VueArray);
                    }
                    for (const key in data) {
                        if (data.hasOwnProperty(key)) {
                            this.ObserveItem(data, key, data[key]);
                        }
                    }
                }
            }
            ObserveItem(data, key, value) {
                this.Observe(value);
                if (!Array.isArray(data)) {
                    this.definePropertpty(data, key, value);
                }
            }

            definePropertpty(data, key, value) {
                const self=this;
                Object.defineProperty(data, key, {
                    get: function () {
            ...
            $set(target, key, value) {
                if (typeof (target) !== "object")
                    return;
                this.ObserveItem(target, key, value);
...
## select
匹配的值为option中的value，如果没有value属性，则为option中的文字 如下面的A、B、C
### 单选
```html
<div id="app">
  <select v-model="selected">
    <option value="s1">A</option>
    <option value="s2">B</option>
...
```
4. vm._watchers:vm上的计算属性和监听的数组,计算属性和侦听器都是Watcher的一个实例 new Watcher(vm,expOrFn,callback,options,isRenderWatcher)
   * vm 实例
   * expOrfn 表达式或者方法 计算属性传递这个参数
   * callback 回调函数 回调函数
   * options	配置项
   * isRenderWatcher 是否是渲染模板的watcher，内部的watcher
...
提供一个在页面上已存在的 DOM 元素作为 Vue 实例的挂载目标。可以是 CSS 选择器，也可以是一个 HTML 元素 实例。 [三种方式](https://blog.csdn.net/xiaodi520520/article/details/88785676)
